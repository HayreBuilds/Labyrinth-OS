...
class MazeSolver:
    """BFS-based maze solver with animated live exploration visualization."""
    ...
    def __init__(self, generator: MazeGenerator):
        ...
        self.path = []
        self.dead_ends = set()
        self.explored = set()  # Track explored cells for live visualization
        self.current_path = [(0, 0)]  # Current path being explored
    ...
    def can_move(self, r: int, c: int, nr: int, nc: int) -> bool:
        ...
        if nc == c + 1 and self.generator.east_wall[r][c] == 0:
            return True
        if nc == c - 1 and self.generator.east_wall[r][nc] == 0:
            return True
        
        return False
    
    def step(self) -> bool:
        """BFS step with live visualization. Returns True if complete."""
        if self.solving_complete:
            return True
        
        if not self.queue:
            # BFS exploration done
            if self.path_found:
                # Path was found - update dead_ends with remaining explored cells
                self.dead_ends = self.explored - set(self.path) - {(0, 0)}
            self.solving_complete = True
            return True
        
        # Dequeue cell and mark as current position
        ...
        if r == self.rows - 1 and c == self.cols - 1:
            ...
            # Mark all explored but non-solution cells as dead ends
            self.dead_ends = self.explored - set(self.path) - {(0, 0)}
            self.path_found = True
            self.solving_complete = True
            return True
        
        # Live visualization: add currently explored cell to path display during exploration
        # This will show orange for dead-ends and blue for solution path later
        self.dead_ends.add((r, c))
        
        # Explore all neighbors in order
        moves = [(r - 1, c),      # North
                 (r, c + 1),      # East
                 (r + 1, c),      # South
                 (r, c - 1)]      # West
        ...
        for nr, nc in moves:
            if self.can_move(r, c, nr, nc) and not self.visited[nr][nc]:
                self.visited[nr][nc] = True
...
class MazeGame:
    ...
    def start_solving(self):
        """Start solving - only if generation is complete."""
        if self.state == GameState.IDLE:
            self.start_generation()
        
        if self.generator.generation_complete:
        ...
        if self.state == GameState.GENERATING:
            for _ in range(self.config.generation_speed):
                if self.generator.step():
                    # Generation complete - don't auto-solve, just stay idle
                    self.state = GameState.IDLE
                    break
            self.gen_steps += 1
...
class MazeUI:
    ...
    def setup_ui(self):
        ...
        # Left side - Maze display (centered)
        ...
        # Container for centered canvas
        canvas_container = tk.Frame(left_frame, bg="#000000")
        canvas_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        
        self.canvas = tk.Canvas(canvas_container, bg="#000000", width=700, height=700, 
        ...
        # Custom maze size section
        self.create_section_label(right_frame, "CUSTOM SIZE")
        size_frame = tk.Frame(right_frame, bg=self.BG)
        size_frame.pack(fill=tk.X, pady=(5, 10))
        
        # Rows input
        tk.Label(size_frame, text="Rows:", font=("Arial", 9), fg=self.FG, bg=self.BG).pack(side=tk.LEFT, padx=5)
        self.rows_var = tk.IntVar(value=20)
        rows_spinbox = tk.Spinbox(size_frame, from_=5, to=100, textvariable=self.rows_var, 
                                 width=5, font=("Arial", 9), bg=self.BTN_BG, fg=self.FG,
                                 command=self.on_size_change)
        rows_spinbox.pack(side=tk.LEFT, padx=2)
        
        # Cols input
        tk.Label(size_frame, text="Cols:", font=("Arial", 9), fg=self.FG, bg=self.BG).pack(side=tk.LEFT, padx=5)
        self.cols_var = tk.IntVar(value=30)
        cols_spinbox = tk.Spinbox(size_frame, from_=5, to=100, textvariable=self.cols_var, 
                                 width=5, font=("Arial", 9), bg=self.BTN_BG, fg=self.FG,
                                 command=self.on_size_change)
        cols_spinbox.pack(side=tk.LEFT, padx=2)
        
        # Main control buttons
        self.create_section_label(right_frame, "GENERATION", top_pad=15)
    ...
    def on_size_change(self):
        ...
        if 5 <= rows <= 100 and 5 <= cols <= 100:
            self.game.new_maze(rows, cols)
            self.cell_pixel_size = 700 / cols
    ...
    def draw(self):
        """Draw maze on canvas with professional rendering and centered maze."""
        self.canvas.delete("all")
        ...
        # Calculate cell size and offset to center the maze
        max_width = 700
        max_height = 700
        cell_width = max_width / gen.cols
        cell_height = max_height / gen.rows
        cell_size = min(cell_width, cell_height)
        
        # Calculate offset to center maze
        maze_width = gen.cols * cell_size
        maze_height = gen.rows * cell_size
        offset_x = (max_width - maze_width) / 2
        offset_y = (max_height - maze_height) / 2
        ...
        self.canvas.create_line(offset_x + gen.cols * cell_size, offset_y, 
                               offset_x + gen.cols * cell_size, offset_y + gen.rows * cell_size, 
                               fill=wall_color, width=wall_thick)
        self.canvas.create_line(offset_x, offset_y + gen.rows * cell_size, 
                               offset_x + gen.cols * cell_size, offset_y + gen.rows * cell_size, 
                               fill=wall_color, width=wall_thick)
        
        # Draw entrance gate (top-left in GREEN)
        entrance_x1 = offset_x
        entrance_y1 = offset_y
        entrance_x2 = offset_x + cell_size
        entrance_y2 = offset_y + cell_size
        self.canvas.create_rectangle(entrance_x1, entrance_y1, entrance_x2, entrance_y2, 
                                    fill="#00ff00", outline="#00ff00", width=2)
        self.canvas.create_text(entrance_x1 + cell_size/2, entrance_y1 + cell_size/2, 
                               text="IN", font=("Arial", 8, "bold"), fill="#000000")
        
        # Draw exit gate (bottom-right in RED)
        exit_x1 = offset_x + (gen.cols - 1) * cell_size
        exit_y1 = offset_y + (gen.rows - 1) * cell_size
        exit_x2 = exit_x1 + cell_size
        exit_y2 = exit_y1 + cell_size
        self.canvas.create_rectangle(exit_x1, exit_y1, exit_x2, exit_y2, 
                                    fill="#ff0000", outline="#ff0000", width=2)
        self.canvas.create_text(exit_x1 + cell_size/2, exit_y1 + cell_size/2, 
                               text="OUT", font=("Arial", 8, "bold"), fill="#ffffff")
        
        # Draw solver path (blue) - show as squares
        if self.game.solver:
            for pr, pc in self.game.solver.path:
                x1 = offset_x + pc * cell_size + 3
                y1 = offset_y + pr * cell_size + 3
                x2 = x1 + cell_size - 6
                y2 = y1 + cell_size - 6
        ...
        if self.game.solver:
            for pr, pc in self.game.solver.dead_ends:
                x1 = offset_x + pc * cell_size + 2
                y1 = offset_y + pr * cell_size + 2
                x2 = x1 + cell_size - 4
                y2 = y1 + cell_size - 4
        ...
        if self.game.solver and self.game.state in [GameState.SOLVING, GameState.COMPLETED, GameState.PAUSED]:
            x = offset_x + self.game.solver.current_col * cell_size + cell_size / 2
            y = offset_y + self.game.solver.current_row * cell_size + cell_size / 2
